...
承認済みワイヤーフレームが存在する場合、以下のゲートも必須とする（DESIGN_RULES §9 参照）:

| ゲート | 目的 | 実行方法 | 失敗時の扱い |
|---|---|---|---|
| ワイヤーフレーム準拠検証 | WFと実装のレイアウト一致 | 目視確認（SOW §4.4 チェックリスト） | ブロック |
...
- 重大インシデントは `docs/governance/runbooks/INCIDENT_CHECKLIST.md` を追加して運用する
- 実装完了後は `RELEASE_CHECKLIST.md` の「ルール適合チェック」を用いて、ソースコードが規約（SEO/Security/Architecture）に沿っているか最終確認する
- **品質証明**: すべてのゲートを通過した後、`docs/governance/requirements/QUALITY_REPORT_TEMPLATE.md` に基づいた品質レポートを作成し、エビデンスとして保存する。

# SOW (Statement of Work) テンプレート

このファイルはテンプレートです。各フェーズごとに複製し、仕様ディレクトリに保存してください（保存先: `HEARING_RULES.md` §1.3）。
- spec-kit + NWG: `specs/<NNN>-<name>/sow_<phase>.md`
- WSK（spec-kit なし）: `docs/projects/<project_slug>/SOW_<phase>.md`

---

## 1. 基本情報
- プロジェクト: <project_slug>
- フェーズ: <phase_name>（例: discovery / design / dev / test / launch）
- 期間: TBD
- 関係者: TBD（責任者/レビュアー/承認者）

## 2. 目的とスコープ
- 目的: TBD
- 含まれる範囲: TBD
- 含まれない範囲（Out of scope）: TBD

## 3. 成果物
- TBD（例: IA案、UIモック、実装ブランチ、テストレポート など）
- ワイヤーフレームが必要な場合の作成方法:
  - A) ユーザー提供物（Figma等）があればそれを基に実装する
  - B) 外部ツール（Google Stitch等）の出力を取り込む場合: 画像→デザインリファレンスとして参照、HTML/コード→`docs/wireframes/`に配置しプロジェクト規約に合わせて再実装
  - C) 上記がない場合は実装担当者がリポジトリ内に静的HTMLモックとして作成・配置する
- フロントエンド実装・調整フェーズでは、実装担当者がブラウザ上で動作/レイアウト/コンソールログを確認しながら作業することを成果物・作業手順に含める（AIエージェント利用時は `chrome-devtools-mcp` / Playwright 等の自動確認ツールを推奨）

## 4. 受入基準チェックリスト（Definition of Done）

*要件定義書および機能仕様書の各項目との対応を明示し、検証方法を紐づける。完了時に `[x]` を記入する。*

### 4.1 機能要件

| 要件ID | 要件概要 | 検証方法 | 完了 |
|---|---|---|---|
| FR-001 | （例: お問い合わせフォーム送信） | （例: E2Eテスト: contact.spec.ts） | [ ] |
| FR-002 | | | [ ] |

### 4.2 非機能要件

| 要件ID | 要件概要 | 検証方法 | 完了 |
|---|---|---|---|
| NFR-001 | （例: CWV Good圏内） | （例: Lighthouse CI） | [ ] |
| SEC-001 | （例: CSP適用確認） | （例: ヘッダー検査スクリプト） | [ ] |

### 4.3 品質ゲート

- [ ] QUALITY_GATES の必須項目を通過（`DEV_RULES §5` に従い `docker compose run --rm <service>` 経由で実行）
  - 例: `docker compose run --rm frontend npm run lint`
  - 例: `docker compose run --rm frontend npm run build`
  - 例: `docker compose run --rm gitleaks`
- [ ] フェーズ特有の完了条件:（例: 主要画面モック3枚承認、E2Eシナリオ5本パス など）

### 4.4 ワイヤーフレーム準拠チェック（承認済みWFがある場合は必須）

*承認済みWF（`docs/wireframes/` 等）の各ページについて、レイアウト検証項目を列挙する（DESIGN_RULES §9 参照）。*
*各項目にWFの具体的な要素（カラム数、配置順序、フォーム項目等）を記載し、検証の具体性を担保する。*

| ページ | 検証項目 | WF参照 | 完了 |
|--------|---------|--------|------|
| TBD | TBD | TBD | [ ] |

## 5. 前提・依存
- TBD（例: 要件確定、API仕様、ブランドガイドの提供、ワイヤーフレーム/UIモック承認済みなど）
- ※実装フェーズのSOWでは、承認済みのワイヤーフレーム/UIモックを前提条件として明記すること（設計フェーズSOWの成果物として承認されたものを参照）

## 6. リスクと緩和策
- 項目: リスク内容 / 影響 / 対応方針 / 期限
- TBD

## 7. 変更管理
- スコープ変更時の手続き: TBD（合意フロー、記録場所）

## 8. コミュニケーション/レビュー体制
- 定例/レビュー頻度、チャネル、承認者: TBD

## 9. 参照
- 要件定義書: 仕様ディレクトリ内の requirements.md（`HEARING_RULES.md` §1.3）
  - spec-kit + NWG: `specs/<NNN>-<name>/requirements.md`
  - WSK: `docs/projects/<project_slug>/REQUIREMENTS_<project_slug>.md`
- 関連ドキュメント: TBD

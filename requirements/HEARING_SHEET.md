...
このシートは **要件定義の入力フォーム** です。未記入がある場合、ヒアリング実施者がプロジェクトオーナーへ質問して埋めます（推測で埋めない）。
...
- **要件定義フェーズでは質問を段階的に進める（一度に大量の質問を列挙しない）。合意済み事項は繰り返し確認しない。**
- 重要度は `Must / Should / Could / Won't` のいずれか
- “国際標準 / Google推奨 / 定石”がある場合はそれを優先する
- APIキー・トークン等の秘密はシートに直接記載しない。取得状況（取得済/未取得）と保管場所のみ記入する。秘密値の配置は権限を持つ担当者が適切な環境変数ファイルに行う。未取得はTBDのまま残し、実装開始前に再確認する。
...
**※選択した「プロジェクト種別」に応じて、適切な構成案を提示・検討すること。**
...
| 2 | Maps API キーの取得状況（取得済 / 未取得）※取得済の場合、担当者が環境変数ファイルに設定 | TBD | Must |
...
5. 発行したキーを担当者が `.env.local` 等の環境変数ファイルに設定する（Git 管理外）
...
§0 で確認したプロジェクト種別に応じて、よくある追加要望を例示しながら確認すること。
...
| 保存に必要なキー/認証情報は準備できるか（取得済 / 未取得）※取得済の場合、担当者が環境変数ファイルに設定 | TBD | Should |
...
| （作成する場合）作成方法：A) 既存デザイン提供（Figma等URL）/ B) 外部ツールで生成して取り込み（Google Stitch等）/ C) 実装担当者が静的HTMLモックで作成 | TBD | Should |
...
- **詳細版**（Hi-fi）: カラー・フォント・実画像を含む、ほぼ完成版の画面。実装担当者が最終デザインまで担当する場合や、デザイナーがいないプロジェクトに適する。

**ワイヤーフレーム作成方法の判断基準:**
- **A) 既存デザイン提供**: Figma等で作成済みのワイヤーフレームがあればそれを基に実装する。
- **B) 外部ツールで生成**: Google Stitch等のデザインツールで生成したワイヤーフレームを取り込む場合。出力形式に応じて、画像ならデザインリファレンスとして参照、HTML/コード出力なら `docs/wireframes/` に配置して実装に取り込む。
- **C) 実装担当者が作成**: 既存デザインも外部ツール出力もない場合、実装担当者がリポジトリ内に静的HTMLモックとして作成・配置する。
...
| **サイトタイトル（title）**<br>※未定の場合、KPI・USP・ターゲットを踏まえた案を検討・提示すること | TBD<br>（30文字前後推奨） | Must | 要件定義時 |
| **サイト説明文（description）**<br>※未定の場合、サイト目的・強みを踏まえた案を検討・提示すること | TBD<br>（PC120文字/スマホ50文字程度推奨） | Must | 要件定義時 |

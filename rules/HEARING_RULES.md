...
- **段階的に進める**: 一度に全ての質問を列挙せず、対話的に進めること
- **合意済みの要件は再質問しない**: ヒアリング済みの項目を繰り返し確認しない
- **推測禁止**: 回答が得られていない項目を推測で埋めてはならない
...
- **ワイヤーフレーム作成が合意されている場合、タスク生成（`/speckit.tasks` の Phase 3+）の前にワイヤーフレームを作成・承認すること**。ワイヤーフレーム未承認の状態で実装タスク（Phase 3+）を生成してはならない（DESIGN_RULES §8.4 参照）

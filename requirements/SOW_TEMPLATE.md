...
- ワイヤーフレームが必要な場合の作成方法:
  ...
  - B) 外部ツール（Google Stitch等）の出力を取り込む場合: 画像→デザインリファレンスとして参照、HTML/コード→`docs/wireframes/`に配置しAIがプロジェクト規約に合わせて再実装
  - C) 上記がない場合はAIエージェントがリポジトリ内に静的HTMLモックとして作成・配置する。ユーザーにFigma等での新規作成を要求しない

...
| パブリック API | 用途に応じて設計 | proxy またはサーバーサイドで制限 |

> **注意**: レート制限の値はプロジェクト要件に応じて調整する。上記は最低基準。

> **レート制限と二重送信防止の違い**: レート制限は分〜時間単位のスパム・不正利用を防ぐ仕組みであり、秒単位の連打（二重送信）には対応できない。フォーム送信・決済・登録など副作用を伴う操作では、クライアント側のボタン無効化 + サーバー側の冪等性保証を別途実装すること（`DEV_RULES §4.2` 参照）。
...
### 7.1 Basic 認証（ステージング保護等）

ステージング環境や社内限定ページで Basic 認証を実装する場合、以下の方針に従うこと:

#### 実装方針

- **`middleware.ts` は使用禁止**。Next.js 16 では `proxy.ts` に統一されている（`DEV_RULES §8.3` 参照）
- Basic 認証は **多層防御** で実装すること。`proxy.ts` 単体に依存しない

#### 多層防御パターン

| レイヤー | 役割 | 実装例 |
|---|---|---|
| **proxy.ts** | 楽観的ゲートキーピング | `Authorization` ヘッダーの存在チェック。無ければ `401 + WWW-Authenticate` を返す |
| **Server Component / Route Handler** | 認証の実体 | トークンのデコードと資格情報の検証を実施 |

> **なぜ `proxy.ts` 単体では不十分か**: CVE-2025-29927 のような脆弱性で proxy 層が bypass された場合、下流のサーバーコンポーネントで認証が行われないとアプリケーションが完全に無防備になる。
